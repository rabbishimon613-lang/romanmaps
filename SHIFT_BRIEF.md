...
- **Auxiliary forts.** Hundreds. Concentrate on frontier zones — Hadrian's Wall is being planned (not built until 122) but many pre-existing forts on that line already existed (Vindolanda is one, already in). Rhine limes: Saalburg, Zugmantel, Kapersburg. Danube limes: Regensburg (Castra Regina, Trajanic), Aquincum (in), Carnuntum (in), Viminacium. **Correction, 2026-09-02, Shift 90:** this section previously called Fossatum Africae "Trajanic so it's fresh" — that's wrong. Construction only began after Hadrian's 122 CE tour of Africa, so any Fossatum Africae feature belongs on the map with `extant_117ce: false` and a note explaining it postdates this snapshot, not as a fresh-construction highlight. See the corrected line below.
...
- **Frontier lines as continuous features.** Fossatum Africae (begun after Hadrian's 122 CE African tour — postdates this snapshot, log `extant_117ce: false`), Limes Germanicus (extended by Domitian, still going in 117), Dacian ripa (Trajan just built it 105–117 — brand new). Draw as `LineString` in a new `frontiers.geojson`.
